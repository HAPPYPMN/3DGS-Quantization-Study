...
def build_rotation(r):
    ...
    R = torch.zeros((q.size(0), 3, 3)).to(q)
...
def build_scaling_rotation(s, q):
    L = torch.zeros((s.shape[0], 3, 3)).to(s)
    R = build_rotation(q)
...
def build_covariance_2d(
    mean3d, cov3d, viewmatrix, fov_x, fov_y, image_width, image_height
):
